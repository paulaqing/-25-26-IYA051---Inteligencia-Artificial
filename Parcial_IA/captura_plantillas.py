...
import cv2
import os
from config import CARD_W, CARD_H, ROI_VALOR, ROI_PALO, PLANTILLAS_VAL, PLANTILLAS_PAL, CAPTURAS_DIR, AREA_MIN
...
def main():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            print("Guardada carta:", ruta)
            contador += 1
        elif key == ord('v') and 'roi_val' in locals():
            etiqueta = input("Nombre valor: ").strip()
            if etiqueta:
                binv = binarizar_para_plantilla(roi_val)
                cv2.imwrite(os.path.join(PLANTILLAS_VAL, f"{etiqueta}.png"), binv)
                print("Guardada plantilla valor:", etiqueta)
        elif key == ord('p') and 'roi_pal' in locals():
            etiqueta = input("Nombre palo: ").strip()
            if etiqueta:
                binp = binarizar_para_plantilla(roi_pal)
                cv2.imwrite(os.path.join(PLANTILLAS_PAL, f"{etiqueta}.png"), binp)
                print("Guardada plantilla palo:", etiqueta)
