...
def binarizar_roi_color(roi_color):
    ...
    # Invertir: símbolo blanco, fondo negro
    ...
    return th  # SIN redimensionar, manteniendo tamaño real de ROI
...
def reconocer_roi(roi_color, plantillas, umbral=UMBRAL_MATCH, topn=3):
    ...
    for nombre, tpl in plantillas.items():
        # Redimensionar plantilla solo para correlacionar
        tpl_r = cv2.resize(tpl, roi_bin.shape[::-1], interpolation=cv2.INTER_AREA)
        # Suavizado ligero
        tpl_s = cv2.GaussianBlur(tpl_r, (3,3), 0)
        roi_s = cv2.GaussianBlur(roi_bin, (3,3), 0)
        res = cv2.matchTemplate(roi_s, tpl_s, cv2.TM_CCOEFF_NORMED)
